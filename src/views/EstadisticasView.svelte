<script lang="ts">
	...
	const balanceActual = $derived(mesActualData.ventas_importe + mesActualData.abonos_importe - mesActualData.compras_importe);
	const balancePrevio = $derived(mesPrevioData.ventas_importe + mesPrevioData.abonos_importe - mesPrevioData.compras_importe);
					...
					datasets: [
						{
							...
							data: [mesActualData.ventas_importe, mesActualData.compras_importe, mesActualData.abonos_importe],
						...
						},
						{
							...
							data: [mesPrevioData.ventas_importe, mesPrevioData.compras_importe, mesPrevioData.abonos_importe],
				...
				data: {
					...
					datasets: [
						...
						},
						{
							...
							data: abonosActual,
							backgroundColor: abonosActual.map((_, i) =>
								i === mesActual ? 'rgba(139,92,246,0.85)' : 'rgba(139,92,246,0.22)'
							),
							borderColor: abonosActual.map((_, i) =>
								i === mesActual ? 'rgba(139,92,246,1)' : 'rgba(139,92,246,0.4)'
							),
							borderWidth: 1,
							borderRadius: 2,
						},
						{
							...
							data: abonosAnt,
							backgroundColor: abonosAnt.map((_, i) =>
								i === mesActual ? 'rgba(139,92,246,0.35)' : 'rgba(139,92,246,0.13)'
							),
		...
		{:else if datosActual}
			...
			<div class="bg-surface rounded border border-slate-200 overflow-hidden animate-fadeSlide" style="animation-delay: 80ms">
				...
				<div class="grid grid-cols-3 divide-x divide-slate-100">
					...
					<!-- Col 3: Cobranza (abonos CXC) -->
					<div class="flex flex-col">
						<div class="grid grid-cols-[minmax(0,auto)_1fr_1fr] px-3 sm:px-4 py-2.5 border-b border-slate-50 items-center">
							<span class="text-[12px] text-slate-600 pr-3">Abonos CXC</span>
							<span class="font-mono text-[12px] text-violet-600 text-right">{formatMXN(mesActualData.abonos_importe)}</span>
							<span class="font-mono text-[12px] text-slate-400 text-right">{formatMXN(mesPrevioData.abonos_importe)}</span>
						</div>
						<div class="mt-auto grid grid-cols-[minmax(0,auto)_1fr_1fr] px-3 sm:px-4 py-2.5 border-t border-slate-200 bg-violet-500/[0.04] items-center">
							<span class="text-[11px] font-mono font-bold text-slate-500 uppercase tracking-wider pr-3">Total</span>
							<span class="font-mono text-[13px] font-bold text-violet-600 text-right">{formatMXN(mesActualData.abonos_importe)}</span>
							<span class="font-mono text-[12px] text-slate-400 text-right">{formatMXN(mesPrevioData.abonos_importe)}</span>
						</div>
					</div>
				...
				<div class="border-t-2 border-slate-200 px-4 py-2.5 flex items-center justify-between bg-bg/40">
					<span class="text-[11px] font-mono font-bold text-slate-500 uppercase tracking-wider">Balance General</span>
