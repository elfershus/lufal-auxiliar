...
<div class="min-h-screen bg-bg flex items-center justify-center p-4">
	<div class="w-full max-w-sm">
		...
		<div class="text-center mb-8">
			<div class="w-16 h-16 rounded-2xl bg-navy flex items-center justify-center mx-auto mb-4">
				<svg class="w-8 h-8 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round">
					<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" />
				</svg>
			</div>
			<h1 class="font-barlow-condensed text-[28px] font-bold text-navy leading-none">Lufal Órdenes</h1>
		...
		<div class="bg-surface rounded-card p-6 shadow-card">
			<p class="text-[12px] text-slate-500 mb-5">
				Ingresa los datos de conexión para continuar. Sólo se requiere configurar una vez.
			</p>

			<form onsubmit={handleSubmit} class="flex flex-col gap-4">
				<!-- Endpoint -->
				<div>
					<label for="setup-endpoint" class="block text-[11px] font-semibold uppercase tracking-[0.1em] text-slate-400 mb-1.5">
						Servidor gRPC
					</label>
					<input
						id="setup-endpoint"
				...
				<!-- API Key -->
				<div>
					<label for="setup-apikey" class="block text-[11px] font-semibold uppercase tracking-[0.1em] text-slate-400 mb-1.5">
						API Key
					</label>
					<div class="relative">
						{#if showKey}
